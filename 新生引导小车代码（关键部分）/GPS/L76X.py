...
class L76X(object):
    ...
    def __init__(self):
        self.config1 = config1.config1(9600)
    ...
    def L76X_Send_Command(self, data):
        ...
        data = data + Temp[int(Check%16)]
        self.config1.Uart_SendString(data.encode())
        self.config1.Uart_SendByte('\r'.encode())
        self.config1.Uart_SendByte('\n'.encode())
        print (data)
        
    def L76X_Gat_GNRMC(self):
        data = self.config1.Uart_ReceiveString(BUFFSIZE)
        print (data)
        print ('\n')
        add=0
        self.Status = 0
    ...
    def L76X_Set_Baudrate(self, Baudrate):
        self.config1.Uart_Set_Baudrate(Baudrate)

    def L76X_Exit_BackupMode(self):
        GPIO.setup(self.config1.FORCE, GPIO.OUT)
        time.sleep(1)
        GPIO.output(self.config1.FORCE, GPIO.HIGH)
        time.sleep(1)
        GPIO.output(self.config1.FORCE, GPIO.LOW)
        time.sleep(1)
        GPIO.setup(self.config1.FORCE, GPIO.IN)
